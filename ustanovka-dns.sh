...
INST_SUM="b3cfda39a64cfc3578677a69068ee8c422f92dd65197503003ca5e4956f33ce5"; INST_SZ="38514"
INIT_SUM="27a10302d85a21f7b37f1eacf9b4e2a4fe8ac057d41cdba99c677a0d96f5db66"; INIT_SZ="30211"
HOOK_SUM="3f7e544c82998482923b9c32bb2ac24d35bd739770e5f62ae4ded1e680254daa"; HOOK_SZ="6484"
...
sverit() {
  # $1 файл, $2 ждём sha, $3 ждём размер, $4 имя для человека
  _z=$(wc -c < "$1"); _s=$(sha256sum "$1" | cut -d' ' -f1)
  say "  $4: размер $_z (ждали $3), sha $(echo "$_s" | cut -c1-16)"
  if [ "$_z" != "$3" ]; then say "    РАЗМЕР не сошёлся"; return 1; fi
  if [ "$_s" != "$2" ]; then say "    СУММА не сошлась"; return 1; fi
  return 0
}

say "=== УСТАНОВКА meridian-dns 0.16, $(date '+%F %T') ==="
mkdir -p "$T" "$BAKDIR"
...
case "$ARCH" in
  ...
  arm64)  DSUM="0949a2f891bcfb461e6900aaad94a240e2920b06c339a2019b500a1f32fc40da"; DSZ="6750370"; IPARCH="aarch64-3.10" ;;
  armv7)  DSUM="84d8d8514cdef120ef373dc4ac74c79df392955d5d7652c06f1f866859cae5fa"; DSZ="7078050"; IPARCH="armv7-3.2" ;;
  mips)   DSUM="37305800aabeaf40730d51f056798a23ee603cd13454f84fab01a34e2b0d3f1e"; DSZ="8061121"; IPARCH="mips-3.4" ;;
  mipsle) DSUM="687c9e5e4bbd741918ba9fe3f0f2c6065c9841a6bb2f607f2b92de70416a2a8d"; DSZ="8061121"; IPARCH="mipsel-3.4" ;;
esac

# ---------- 2. ЗАГРУЗКА НАБОРА ----------
say "--- загрузка набора DNS 0.16 ($SUF)"
DNSNEW="$T/meridian-dns"; INSTNEW="$T/install.sh"; INITNEW="$T/init.sh"; HOOKNEW="$T/hook.sh"
RC=0
skachat "$BASE/meridian-dns-$ARCH$SUF" "$DNSNEW"; if [ $? -ne 0 ]; then RC=1; fi
skachat "$BASE/meridian-dns-install.sh$SUF" "$INSTNEW"; if [ $? -ne 0 ]; then RC=1; fi
skachat "$BASE/meridian-dns-init.sh$SUF" "$INITNEW"; if [ $? -ne 0 ]; then RC=1; fi
skachat "$BASE/meridian-dns-netfilter-hook.sh$SUF" "$HOOKNEW"; if [ $? -ne 0 ]; then RC=1; fi
...
BAD=0
sverit "$DNSNEW" "$DSUM" "$DSZ" "сервер ($ARCH)"; if [ $? -ne 0 ]; then BAD=1; fi
sverit "$INSTNEW" "$INST_SUM" "$INST_SZ" "установщик"; if [ $? -ne 0 ]; then BAD=1; fi
sverit "$INITNEW" "$INIT_SUM" "$INIT_SZ" "init"; if [ $? -ne 0 ]; then BAD=1; fi
sverit "$HOOKNEW" "$HOOK_SUM" "$HOOK_SZ" "крючок"; if [ $? -ne 0 ]; then BAD=1; fi
if [ "$BAD" != 0 ]; then
...
postavit_ipset() {
  ...
  for row in "libmnl_1.0.5-1 libmnl" "libipset_7.24-1 libipset" "ipset_7.24-1 ipset"; do
    ...
    f="$T/$short.ipk"
    skachat "$BASE/${n}_${IPARCH}${IPSUF}.ipk" "$f"
...
if [ "$PRC" = 0 ]; then
  ...
  say "ГОТОВО: DNS 0.16 установлен и пережил перезапуск."
  rm -rf "$T"
  exit 0
fi
if [ "$PRC" != 1 ]; then
